function [] = simulateAllCombs555(identifier)
%like version 444, but we adapt the proposal distribution
ppar = csvread(sprintf('sim%d_paramsMeansNoise.csv',identifier));
fHandle          = @pseudoRankIndivParams;  
fileName         = sprintf('sim%dNoise.csv',identifier);
...
tic
for k = 31:35
permutationFileName = sprintf('sim%dNoise_Permutation%d.csv',identifier,k+95);
feval(fHandle, fileName, k+100, nSamples, logPriorGPMeans,logPriorGPSds, ...
    ...
    regInt,permutationFileName,true,NaN,burnIn,false,false)
end
toc
%move 2
